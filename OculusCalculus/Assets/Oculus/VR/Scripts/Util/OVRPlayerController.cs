...
public class OVRPlayerController : MonoBehaviour
{
	...
	void Start()
	{
        ...
        CameraRigPosition = CameraRig.transform.localPosition;
		...
		if (EnableLinearMovement)
		{
			...
			if (primaryAxis.x > 0.0f)
				MoveThrottle += ort * (primaryAxis.x * transform.lossyScale.x * moveInfluence * BackAndSideDampen *
									   Vector3.right);
            //up<->down
            if(OVRInput.Get(OVRInput.Button.PrimaryHandTrigger) && OVRInput.Get(OVRInput.Button.PrimaryThumbstickUp))
            {
                CameraRigPosition.y += 0.01f;
                CameraRig.transform.localPosition = CameraRigPosition;
            }
            if (OVRInput.Get(OVRInput.Button.PrimaryHandTrigger) && OVRInput.Get(OVRInput.Button.PrimaryThumbstickDown))
            {
                CameraRigPosition.y -= 0.01f;
                if (CameraRigPosition.y < 0f) CameraRigPosition.y = 0f;
                CameraRig.transform.localPosition = CameraRigPosition;
            }
        }
